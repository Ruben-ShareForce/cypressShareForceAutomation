describe("Deactivate Account", () => {
  ...
  it("can filter for active employees and deactivate their account", () => {
    cy.visit("/");
    ...
    cy.contains("a", "Deactivate Account").should("be.visible").click();
    cy.wait(1000);

    cy.contains("span", "Yes").should("be.visible").click();
    cy.wait(500);

    cy.get("div.alert.alert-dismissable.alert-info").should("be.visible").contains("The account has been deactivated");
  });
